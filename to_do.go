package massa

// ------------------------------------------------------------
// --------------- TODO list before publishing: ---------------
// ------------------------------------------------------------
//
//	- Add comments to all public methods for GoDoc.
//
//	- Design and implement wallet service so we can separate
//	  wallet management/handling from wallet functions.
//		- Add Wallet interface so we can standardize behaviour
//		  across multiple wallet implementations (eg; standard
//		  wallet and Heriarchical Deterministic wallet.)
//
//	- Add more Api Client methods.
//		- Get datastore entries.
//
//	- Better test coverage.
//
//	- More examples.
//
//	- Add argument utils for smart contract params.
//
//	- Unify different components under one Massa Client.
//
//	- Top level transactions and smart contract calls.
//
